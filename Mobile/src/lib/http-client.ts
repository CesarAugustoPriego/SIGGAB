...
import { ApiClientError, type ApiEnvelope, type ApiErrorPayload } from '@/src/types/api';

import { clearSession, readSession, writeSession, type AuthSession } from './session-storage';
...
function buildHeaders(path: string, withAuth: boolean, accessToken?: string, customHeaders?: Record<string, string>) {
...
async function executeRequest<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    if (response.ok) {
      if (!payload?.success) {
        throw new ApiClientError('Respuesta inválida del servidor', response.status);
      }
    ...
    if (
      ...
      if (refreshedTokens?.accessToken) {
        ...
        return executeRequest<T>(path, {
          ...options,
          retryOn401: false,
          accessTokenOverride: nextSession.accessToken,
        });
      }
