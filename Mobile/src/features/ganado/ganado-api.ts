...
export const ganadoApi = {
  ...
  getAnimalById: (idAnimal: number) => httpClient.get<Animal>(`/animales/${idAnimal}`),

  getAnimalByArete: (numeroArete: string) => httpClient.get<Animal>(`/animales/arete/${encodeURIComponent(numeroArete)}`),
